...
from core.globals import PLANNER_CHAIN
...
async def planner_node(state:SalesState):
    logger.info("👉 Planner Agent（LLM决策）")
    long_memory = await get_long_memory(state["session_id"])
    history = await get_user_memory(state["session_id"])
    response:Plan = await PLANNER_CHAIN.ainvoke({"input":state["input"],"history":history,"long_memory":long_memory})
    return {
        "task": response.tasks,
    }
