...
from core.globals import SCORE_CHAIN
...
async def score_node(state:SalesState):
    logger.info("👉 score_node")
    history = await get_user_memory(state["session_id"])
    long_memory = await get_long_memory(state["session_id"])
    response:Score = await SCORE_CHAIN.ainvoke({"text":state["input"],
