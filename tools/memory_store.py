import hashlib
import json
from core.globals import UPDATE_CHAIN
from model.node_model import Memory
from tools.redis_store import redis_client

MAX_LENGTH = 15
async def save_user_memory(session_id,role,content):
...
async def get_user_memory(session_id):
    s_id = f"session:{session_id}"
    messages = redis_client.lrange(s_id,0,MAX_LENGTH-1)
    messages.reverse()
    return [json.loads(item) for item in messages]

async def clear_user_memory(session_id):
    s_id = f"session:{session_id}"
    redis_client.delete(s_id)

async def save_rag_memory(session_id,text,docs:list):
    rag_id = f"rag:{session_id}"
    redis_client.hset(rag_id,text,json.dumps(docs))

async def get_rag_memory(session_id,text):
    rag_id = f"rag:{session_id}"
    docs = redis_client.hget(rag_id,text)
    if docs:
        return json.loads(docs)
    return []
async def clear_rag_memory(session_id):
    rag_id = f"rag:{session_id}"
    redis_client.delete(rag_id)

async def make_key(module: str, session_id: str, text: str):
    text_hash = hashlib.md5(text.encode()).hexdigest()
    return f"{module}:{session_id}:{text_hash}"

async def save_node_memory(session_id,node_name,text,docs:list):
    node_id = await make_key(node_name,session_id,text)

    redis_client.set(node_id,text)
    redis_client.expire(node_id,3600)

async def get_node_memory(session_id,node_name,text):
    node_id = await make_key(node_name,session_id,text)
...
async def get_long_memory(session_id):
    key = f"session:{session_id}:profile"
    data = redis_client.get(key)
    if not data:
        return {}
    return json.loads(data)
async def save_long_memory(session_id,data):
    key = f"session:{session_id}:profile"
    redis_client.set(key,json.dumps(data),ex=7*24*3600)


async def update_long_memory(session_id,history):
    key = f"session:{session_id}:profile"
    response:Memory = UPDATE_CHAIN.ainvoke({"history":history})
    try:
